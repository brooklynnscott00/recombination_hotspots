...
#SBATCH --job-name="count the number of hits in 20kb windows forhap1"
...
for scaffold in HiC_scaffold_{1..21}; do
  grep '^MOTIF' PRDM9_motifs.human.txt | cut -f 2 -d ' ' | while read MOTIF; do
    ...
    bedtools intersect \
      ...
      -b <(grep -P "^$scaffold\t" hap1_fimo/TID_1039885_H1.PRDM9.fixed.bed | grep -P "$MOTIF\t" | awk -v max_qvalue=$max_qvalue '$8 <= max_qvalue') -c | \
      awk -v OFS='\t' -v motif=$MOTIF '{print($0,motif)}' \
      >> hap1_PRDM9_hits/TID_1039885_H1.PRDM9.${window_size}.bed
  done
done
